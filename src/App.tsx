...
function App() {
  ...
  const loadOrgs = async (tokenOverride?: string) => {
    ...
    setOrgsError(null)
    try {
      const data: { organization: { id: string; name: string }; role: Org["role"]; status: string }[] =
        await apiRequest({ url: "/orgs", method: "GET" }, { tokenOverride })
      ...
      setOrgsError(error instanceof Error ? error.message : "Nao foi possivel carregar as organizacoes")
    } finally {
  ...
  useEffect(() => {
    ...
    apiRequest<{ user: AuthUser }>({ url: "/auth/me", method: "GET" }, { tokenOverride: storedAccess })
      .then((res) => {
        setUser(res.user)
  ...
  useEffect(() => {
    if (!isAuthenticated && (stage === "orgSelect" || stage === "orgCreate" || stage === "app")) {
      setStage("login")
    }
  ...
  const handleLogin = async (email: string, password: string) => {
    ...
    setAuthError(null)
    try {
      const data: { accessToken: string; refreshToken: string; user: AuthUser } = await apiRequest(
  ...
  const handleCreateOrg = async (name: string, domain?: string) => {
    ...
    try {
      const newOrg = await apiRequest<Org>({ url: "/orgs", method: "POST", data: { name, domain } })
      const normalized: Org = { id: newOrg.id, name: newOrg.name, role: "OWNER" }
      setOrgs((prev) => [...prev, normalized])
      persistOrg(normalized.id)
      setStage("app")
    } catch (error) {
      setOrgCreateError(error instanceof Error ? error.message : "Erro ao criar organizacao")
    } finally {
